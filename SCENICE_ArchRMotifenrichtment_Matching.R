...
data.dir <- './MotifenrichmentSCENIC/newMATCH'
dir.create(data.dir)
setwd(data.dir)
...
for (n in 1:25){
  ...
  SCENICtable$relactzscore <- scale(SCENICtable$RelativeActivity)
  ...
  TNTjoinedstring <-  stringdist_join(tabledfTNT, SCENICtableTNT, 
                  ...
                  method = "jw", 
                  #p=0.25,
                  max_dist = 0.9, #this distance determined by manual review of the matches
                  distance_col = "dist") %>%
    group_by(TF.x) %>%
  ...
  TNTjoinedstring2 <- TNTjoinedstring %>% filter(substring.x==substring.y)
  ...
  ggp <- ggplot(TNTjoinedMAT, aes(TF.y,TF.x)) +                           # Create heatmap with ggplot2
    ...
    ggtitle(paste0("Aggregated Regulon-/Motifenrichment-Z-Score for respective Transscriptionfactors TnT - Cluster ",n)) +
  ...
  MHCjoinedstring <-  stringdist_join(tabledfMHC, SCENICtableMHC, 
                                      ...
                                      ignore_case = FALSE, 
                                      method = "jw", 
                                      max_dist = 0.8, #this distance determined by manual review of the matches
                                      distance_col = "dist") %>%
    group_by(TF.x) %>%
    slice_min(order_by = dist, n = 1)
  ...
  ggp <- ggplot(MHCjoinedMAT, aes(TF.y,TF.x)) +                           # Create heatmap with ggplot2
    ...
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    xlab("Enriched Motifs in Chromatin accessibility data") + ylab("Active Regulons")+
    ggtitle(paste0("Aggregated Regulon-/Motifenrichment-Z-Score for respective Transscriptionfactors MyHC - Cluster ",n)) +
    theme(plot.title = element_text(hjust = 0.5))+ labs(fill = "Combined Z-Score") + theme(legend.title.align=0.5)
