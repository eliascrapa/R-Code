...
data.dir <- './comparison_MHCvsTNT_woWeights'
dir.create(data.dir)
setwd(data.dir)
...
object.list <- list(MHC = cellchat_MHC,TNT = cellchat_TNT)
...
ggsave(filename=paste0("MHCvsTNTNumber_of_Interaction_Comparison.png"), plot= arrange, width = 10, height = 8, units = 'in', dpi = 300)
...
png("MHCvsTNT_Network.png", width = 960, height = 960, units = "px")
print(netVisual_diffInteraction(cellchat, weight.scale = T))
dev.off()

png("MHCvsTNT_Network_weight.png", width = 960, height = 960, units = "px")
print(netVisual_diffInteraction(cellchat, weight.scale = T, measure = "weight"))
dev.off()

#PDFs of same plots
pdf("MHCvsTNT_Network.pdf")
print(netVisual_diffInteraction(cellchat, weight.scale = T))
dev.off()

pdf("MHCvsTNT_Network_weight1122.pdf")
print(netVisual_diffInteraction(cellchat, weight.scale = T, measure = "weight"))
dev.off()
...
pdf("MHCvsTNT_VisualHeatmap.pdf")
par(mfrow = c(1,2), xpd=TRUE)

print(netVisual_heatmap(cellchat))
dev.off()

pdf("MHCvsTNT_VisualHeatmap_weight.pdf")
...
for (i in 1:length(object.list)) {
  png(paste0("MHCvsTNT_Interactions,",i,".png"), width = 960, height = 960, units = "px") 
...
pdf("MHCvsTNT_DiffinterationNumberandStrength.pdf")
par(mfrow = c(1,2))#, xpd=TRUE)
...
pdf("MHCvsTNT_MajorSourcesandTargesIn2DSpace.pdf")
...
for (n in 1:25){
pdf(paste0("MHCvsTNT_SignalingChanges_Scatterplot_C",n,".pdf"))
...
ggsave(gg1, device = "pdf", filename = "Pathwaysimilarities_TNT_MHC.pdf")
...
#> Classification learning of the signaling networks for datasets 1 2
# Visualization in 2D-space
#pdf("MHCvsTNT_Signalinggroups_basedon_structural similatries.pdf")
gg1 <- netVisual_embeddingPairwise(cellchat, type = "structural", label.size = 8, top.label = T)
ggsave(gg1, device = "pdf", filename = "MHCvsTNT_Signalinggroups_basedon_structural similatries.pdf")
#dev.off()
#> 2D visualization of signaling networks from datasets 1 2
#pdf("MHCvsTNT_Signalinggroups_basedon_structural similatries_Pairwisezoomin.pdf")
gg1 <- netVisual_embeddingPairwiseZoomIn(cellchat, type = "structural", nCol = 2,
                                         show.legend = T)
ggsave(gg1, device = "pdf", filename = "MHCvsTNT_Signalinggroups_basedon_structural similatries_Pairwisezoomin.pdf")
#> 2D visualization of signaling networks from datasets 1 2
#dev.off()
...
#pdf("MHCvsTNT_Difference_in_Pathwaydistance_higher_Score more different between both groups.pdf")
gg1 <- rankSimilarity(cellchat, type = "functional")
ggsave(gg1, device = "pdf", filename = "MHCvsTNT_Difference_in_Pathwaydistance_higher_Score more different between both groups.pdf")
#dev.off()
...
#pdf("MHCvsTNT_Informationflow_of_each_pathways_comparison.pdf")
gg <- grid.arrange(gg1 + gg2, nrow = 1)
ggsave(gg, device = "pdf", filename = "MHCvsTNT_Informationflow_of_each_pathways_comparison.pdf")

#dev.off()

save.image("Cellchat_woweights_MHCvsTNT.Rdata")
...
pdf("MHCvsTNT_Outgoing_Signalpattern_Comparison_Heatmap.pdf")#, paper = "a3")
draw(ht1 + ht2, ht_gap = unit(0.2, "cm"))
#gg1 <- grid.arrange(ht1+ht2)
#ggsave(gg1, device = "pdf", filename = "MHCvsTNT_Outgoing_Signalpattern_Comparison_Heatmap.pdf")
...
pdf("MHCvsTNT_Incoming_Signalpattern_Comparison_Heatmap.pdf")
...
pdf("MHCvsTNT_Overall_Signalpattern_Comparison_Heatmap.pdf")
draw(ht1 + ht2, ht_gap = unit(0.5, "cm"))
dev.off()
...
{
pdf(paste0("MHCvsTNT_ligand-receptor_pairs_Cluster_",sourcecluster,"to_Cluster",paste0(targetcluster, collapse = ","),".pdf"))
...
#> Comparing communications on a merged object
pdf(paste0("MHCvsTNT_Increased_Decreased_Signaling_Comparison_by_communication probabities",sourcecluster,"to_Cluster",paste0(targetcluster, collapse = ","),".pdf"))
gg1 + gg2
dev.off()
...
#> Comparing communications on a merged object
pdf(paste0("MHCvsTNT_Up-Downregulated_Signaling_Comparison_by_differentialGeneExpression",sourcecluster,"to_Cluster",paste0(targetcluster, collapse = ","),".pdf"))
gg1 + gg2
dev.off()
...
# Chord diagram
par(mfrow = c(1,2), xpd=TRUE)
pdf(paste0("MHCvsTNT_Up-Downregulated_Signaling_Comparison_by_differentialGeneExpression_Chordplot",sourcecluster,"to_Cluster",paste0(targetcluster, collapse = ","),".pdf"))
...
pdf(paste0("MHCvsTNT_Up-Interactionpathways",sourcecluster,"to_Cluster",paste0(targetcluster, collapse = ","),".pdf"))
#
for (i in 1:length(object.list)) {
...
pathways <- intersect(object.list[["MHC"]]@netP[["pathways"]],object.list[["TNT"]]@netP[["pathways"]])
...
pdf(paste0("MHC_vs_TNT_Circleplot_",x,"_Pathway.pdf"), paper = "a4r")
par(mfrow = c(1,2), xpd=TRUE)
for (i in 1:length(object.list)) {
...
#> 
#> Do heatmap based on a single object
pdf(paste0("MHC_vs_TNT_Heatmapt_",x,"_Pathway.pdf"), paper = "a4r")
...
# Chord diagram
pdf(paste0("MHC_vs_TNT_Chordplot_",x,"_Pathway.pdf"), paper = "a4r")
...
pdf(paste0("MHC_vs_TNT_Chordplot_grouped_Celltypes_",x,"_Pathway.pdf"), paper = "a4r")
par(mfrow = c(1,2), xpd=TRUE)
for (i in 1:length(object.list)) {
...
cellchat@meta$datasets = factor(cellchat@meta$datasets, levels = c("MHC", "TNT")) # set factor level
...
ggsave(ggq, device = "pdf", filename = paste0("MHCvsTNT_Genexpression_VlnPlot_Singnalingpathway",x,".pdf"))
}
