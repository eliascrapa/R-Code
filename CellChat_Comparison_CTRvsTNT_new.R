...
data.dir <- './comparison_CTRvsTNT_woWeights'
dir.create(data.dir)
setwd(data.dir)
...
ggsave(gg1, device = "pdf", filename = "Pathwaysimilarities_TNT_CTR.pdf")
...
ggsave(gg1, device = "pdf", filename = "CTRvsTNT_Signalinggroups_basedon_structural similatries.pdf")
#dev.off()
#> 2D visualization of signaling networks from datasets 1 2
...
gg1 <- netVisual_embeddingPairwiseZoomIn(cellchat, type = "structural", nCol = 2,
                                         show.legend = T)
ggsave(gg1, device = "pdf", filename = "CTRvsTNT_Signalinggroups_basedon_structural similatries_Pairwisezoomin.pdf")
#> 2D visualization of signaling networks from datasets 1 2
#dev.off()
...
ggsave(gg1, device = "pdf", filename = "CTRvsTNT_Difference_in_Pathwaydistance_higher_Score more different between both groups.pdf")
#dev.off()
...
ggsave(gg, device = "pdf", filename = "CTRvsTNT_Informationflow_of_each_pathways_comparison.pdf")

#dev.off()

save.image("Cellchat_woweights_CTRvsTNT.Rdata")
...
#ggsave(gg1, device = "pdf", filename = "CTRvsTNT_Outgoing_Signalpattern_Comparison_Heatmap.pdf")
...
pathways <- intersect(object.list[["CTR"]]@netP[["pathways"]],object.list[["TNT"]]@netP[["pathways"]])
...
pdf(paste0("CTR_vs_TNT_Circleplot_",x,"_Pathway.pdf"), paper = "a4r")
par(mfrow = c(1,2), xpd=TRUE)
for (i in 1:length(object.list)) {
...
#> 
#> Do heatmap based on a single object
pdf(paste0("CTR_vs_TNT_Heatmapt_",x,"_Pathway.pdf"), paper = "a4r")
...
# Chord diagram
pdf(paste0("CTR_vs_TNT_Chordplot_",x,"_Pathway.pdf"), paper = "a4r")
...
pdf(paste0("CTR_vs_TNT_Chordplot_grouped_Celltypes_",x,"_Pathway.pdf"), paper = "a4r")
par(mfrow = c(1,2), xpd=TRUE)
for (i in 1:length(object.list)) {
...
ggsave(ggq, device = "pdf", filename = paste0("CTRvsTNT_Genexpression_VlnPlot_Singnalingpathway",x,".pdf"))
}
