...
data.dir <- './Markermotifs_Chromvar_norm3'
dir.create(data.dir)
setwd(data.dir)
 ...
 p1 <- (ggplot(tabledfTNT, aes(x = reorder(TF, zscore), y = zscore))+
   ...
   scale_fill_manual(values = c("different" ="#3c8cdc", "matching"="#e8c142"))+
    theme(legend.text=element_text(size=12),
          ...
          plot.title = element_text(hjust = 0.85, vjust = - 107, size = 20))#  #, plot.title = element_text(vjust = - 10)#+ #vlegend.position = "none") 
  #scale_fill_discrete(breaks = c("matching", "different"))
 
 
 if(nrow(tabledfTNT)>0) {
   p1 = p1 + geom_text(aes(label=TF, y= 0.05), colour = "black", nudge_x = +0.0, hjust=-0.0)
 }
  ...
  #scale_fill_discrete(breaks = c("matching", "different"))
    ...
    if(nrow(tabledfMHC)>0) {
      p2 = p2 + geom_text(aes(label=TF, y= 0.05), colour = "black", nudge_x = +0.0, hjust=-0.0)
    }
  
  design="11112223"
    
   # pp<- p1 + p2 + guide_area() + plot_layout(design=design, guides = "collect", widths = 50)# & theme(legend.position= c(.7, .8))#c("top"))
    pp<- p1 + p2 + plot_layout(guides = "collect", widths = c(50,50), heights = c(29,1)) & theme(legend.position= ("top"))
